...
public class MeshGenerator : MonoBehaviour {
    ...
    ComputeBuffer maskpointsBuffer;
    private int[,] densityMask;
    ...
    public void UpdateChunkMesh (Chunk chunk) {
        ...
        updateMaskPoints();
        densityGenerator.ModifyWithMask(pointsBuffer, maskpointsBuffer, numPointsPerAxis);
        //test
        for (int i = 0; i < cellularSteps; i++)
            densityGenerator.CellularAutma(pointsBuffer, numPointsPerAxis);
        ...
        //
        // mesh.RecalculateSmoothNormals (0.2f);
        //
    }
    ...
    void CreateBuffers () {
        ...
        if (!Application.isPlaying || (pointsBuffer == null || numPoints != pointsBuffer.count)) {
            ...
            maskpointsBuffer = new ComputeBuffer (numPoints, sizeof (float));
...
#pragma warning disable 649 // disable unassigned variable warning
    ...
    void OnDrawGizmos () {
        if (showBoundsGizmo) {
            ...
            foreach (var chunk in chunks) {
                ...
                //draw cube for voxel  (for debugging)
                int numVoxelsPerAxis = numPointsPerAxis - 1;
                for (int x = 0; x < numVoxelsPerAxis; x++) {
                    // for (int y = 0; y < numPointsPerAxis; y++) {
                        
                            Vector3 pos = new Vector3 (x, 0, 0) * boundsSize / (numVoxelsPerAxis) + CentreFromCoord (chunk.coord) - Vector3.one * boundsSize / 2;
                            pos += Vector3.one * boundsSize / (numVoxelsPerAxis * 2);
                            Gizmos.DrawCube (pos, Vector3.one * boundsSize / (numVoxelsPerAxis));
    ...
    void updateMaskPoints(){
        int numPoints = numPointsPerAxis * numPointsPerAxis * numPointsPerAxis;
        var instanceData = new float[numPoints];
        if(densityMask != null){
            generateMaskPoints(densityMask);
            int[,] map = densityMask;
            ...
            for (int y = 0; y < numPointsPerAxis; y++) {
                for (int z = 0; z < numPointsPerAxis; z++) {
                    for (int x = 0; x < numPointsPerAxis; x++) {
                        int index = z * numPointsPerAxis * numPointsPerAxis + y * numPointsPerAxis + x;
                        float u = (float)x/numPointsPerAxis;
                        float v = (float)z/numPointsPerAxis;

                        int ux = (int)(u * (width - 1));
                        int vy = (int)(v * (height - 1));
                        instanceData[index] = (map[ux,vy]==1)?1f:0.05f;
                        //instanceData[index] = (float)map[ux,vy];
                    }
                }
            }
        ...
        maskpointsBuffer.SetData(instanceData);
        
    }

    public void generateMaskPoints( int[,] map){
        //get dimensions of map
       densityMask = map;
       
    }

    void SmoothMeshNormals(Mesh m)
        {
